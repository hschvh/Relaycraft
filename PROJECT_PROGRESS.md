...
项目用于发布 `Relaycraft` 公开仓库及其 `relay` 技能。当前阶段：已完成技能创建、验证和公开仓库推送；正在处理公开 Git 历史中的真实作者身份脱敏。
...
- 当前远程 `main` 的提交树已确认共有 14 个提交；这些提交仍包含待脱敏的本机作者身份。

## 进行中与未完成

- 重写远程 `main` 的提交作者/提交者为 `Relaycraft <hschvh@users.noreply.github.com>`。
...
2. 创建本地 bundle 备份并记录重写前远程 SHA。
3. 使用化名重写作者/提交者元数据，保留文件树和提交消息。
4. 强制推送 `main`，核对远程提交、文件内容和技能 validator。

## 最近更新时间与变更记录

- 2026-09-05 Asia/Shanghai：确认公开历史暴露真实姓名；创建本进度文件；准备使用化名重写并强制更新远程 `main`。
