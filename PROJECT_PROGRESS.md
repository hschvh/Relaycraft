...
项目用于发布 `Relaycraft` 公开仓库及其 `relay` 技能。当前阶段：README 已加入可点击的中文/English 区块切换，并已提交、推送和核对。
...
- 本地及远程重写后分支共有 21 个提交，全部使用 `Relaycraft <hschvh@users.noreply.github.com>`。
...
- README 顶部和两个语言区块均加入 `#中文` / `#english` 跳转链接；语言切换检查通过。
...
7. 已提交语言切换链接和本进度更新，推送后已重新核对锚点、README 内容与工作树状态。

## 最近更新时间与变更记录

- 2026-09-05 Asia/Shanghai：创建进度文件；完成本地作者元数据重写；强制更新远程 `main`；核对远程 21 个提交均使用化名；新增 README 并推送；将 README 改为中英文双语；加入可点击语言切换链接并完成推送核对。
