...
项目用于发布 `Relaycraft` 公开仓库及其 `relay` 技能。当前阶段：README 已提交、推送并完成本地远程跟踪核对。
...
- 本地及远程重写后分支共有 19 个提交，全部使用 `Relaycraft <hschvh@users.noreply.github.com>`。
...
- 2026-09-05 Asia/Shanghai：创建进度文件；完成本地作者元数据重写；强制更新远程 `main`；核对远程 19 个提交均使用化名；新增 README 并推送；核对远程跟踪分支与本地内容一致。
